					...
					if ( $sanitized_name === 'pa_color' ) {
						...
						foreach ( wc_get_product_terms( $product->get_id(), 'pa_color', [ 'fields' => 'all' ] ) as $term ) {
							if ( in_array( $term->slug, $options, true ) ) {
								...
								echo '<a href="#" class="color-swatch ' . esc_attr( $active ) . '" data-value="' . esc_attr( $term->slug ) . '" style="background: ' . esc_attr( $term->slug ) . ';">';
        ...
        <div class="woocommerce-variation-add-to-cart variations_button">
            ...
            <div class="product-details-action">
                ...
                <div class="details-action-wrapper">
                    <a href="#"
                       class="btn-product btn-wishlist bookmark-product <?php echo tns_user_bookmark_product_compare(get_current_user_id(), $product->get_id()) ? 'liked' : ''; ?>"
...
افزودن به لیست علاقه‌مند
                    ...
                    <?php endif;?>
                    <?php if(get_option('_tnm_set_index_website') === 'in3') : ?>
                      <?php if (get_option('_tnm_settings_set_general')['compare_enable'] === 'yes') : ?>
                    <a href="#" class="btn-product btn-compare bookmark-product-compare <?php echo tns_user_bookmark_product(get_current_user_id(), $product->get_id()) ? 'liked' : ''; ?>"
                       data-pid="<?php echo $product->get_id(); ?>"
                       title="مقایسه">
